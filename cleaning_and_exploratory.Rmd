...
We can also see that the WDI dataset has many NA values, so it probably has a lot of empty rows and/or columns Let's check how many NA values are present in the columns:
...
We finally have our input dataset for the predictive model. However, because our dataset is so large (3860 obs. of 1363 variables) one thing that could be useful is to batch/group our observations according to a categorical variable. This way, we can analyse different groups of countries at a time, breaking up the processing tasks in chunks. We have some complimentary metadata from World Bank regarding the WDI dataset that may be useful:

```{r, load-3}
country_metadata_wdi <- read_csv("dados/WDI_CSV/WDICountry.csv", col_select = c(1, 3, 8, 9), col_types = cols(
  "Country Code" = col_character(),
...
country_metadata_wdi <- country_metadata_wdi %>%
  clean_names() %>%
  dplyr::rename(country = 2)

glimpse(country_metadata_wdi)
```

Now we'll do a left join so that we add the metadata to our main dataset (and also do some cleaning up e.g. reordering of columns):

```{r, merge-metadata-1}
cs_wdi <- merged_cs_wdi %>%
  left_join(country_metadata_wdi, by = c("country_code" = "country_code")) %>%
  select(-country.x, -country.y) %>%
  relocate(cs_section_rate:income_group, .before = year) %>%
  relocate(country, .before = cs_section_rate) %>%
  relocate(weight, .before = cs_section_rate) %>%
  relocate(weight:cs_section_rate, .after = per_si_allsi.cov_q5_tot)

colnames(cs_wdi)
write_csv(cs_wdi, "/Users/andressaflores/Documents/projects/cs_socioeconomic/dados/cs_wdi.csv")
```

We also have some metadata regarding our socioeconomic variables contemplated in columns 8:1364. Since the majority of potential features of our model are continuous variables, we're looking for categorical variables that will allow the grouping:
