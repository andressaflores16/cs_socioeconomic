...
We won't remove `region` since it wasn't considered by Boruta since it didn't have enough categories, but is still interesting to consider in our model, according to the literature. First, we need to split out dataset into train and test:

```{r, input-rf-dataset-4}
set.seed(123)  # for reproducibility

# removing categorical variables that won't be considered in Random Forest training and testing
rf_input <- imp_cs_wdi_top20 %>%
...
train    <- rf_input[split == 'train',]
test     <- rf_input[split == 'test',]
```
...
```{r, random-forest-evaluate}

pRF_train <- predict(rf, train)
pRF_test  <- predict(rf, test)
avalia_regressao(pRF_train, train$cs_section_rate)
avalia_regressao(pRF_test, test$cs_section_rate)
```
```{r}
print(rf$mse)
```
...
rf_spec <- train(
  cs_section_rate ~ . - weight,  # Assuming 'weight' is the name of your weight column
  data = rf_input,
  method = "rf",
  trControl = ctrl,
  metric = "RMSE",  # Or another appropriate metric
  weights = rf_input$weight,  # Specify the weight column
  tuneGrid = data.frame(mtry = c(2, 4, 6, 8)),  # Example tuning grid
  importance = TRUE
)
